...
/* ══════════════════════════════════════════════════════════
   USER PROFILE SCREEN
══════════════════════════════════════════════════════════ */
function UserProfileScreen({ user, onBack, onPostClick, accentColor }) {
  const { posts } = window.APP_DATA;
          ...
          <div style={{ paddingTop: 42 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 17, fontWeight: 700, color: '#1C1815' }}>{user.name}</span>
                <span style={{
            ...
            <div style={{ fontSize: 13, color: '#7A7268', marginBottom: 12 }}>{user.title}</div>

            {/* Stats */}
            <div style={{ display: 'flex', borderTop: '1px solid #EDE8E0', paddingTop: 14 }}>
              {[
                ['发帖',  user.posts],
                ['获赞',  user.likes > 999 ? `${(user.likes/1000).toFixed(1)}k` : user.likes],
                ['粉丝',  user.followers > 999 ? `${(user.followers/1000).toFixed(1)}k` : user.followers],
              ].map(([label, val]) => (
