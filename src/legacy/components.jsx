...
function PostImage({ post, rounded = true }) {
  const previewAspect = post?.imageAspect === 9 / 16 ? 3 / 4 : (post?.imageAspect || 1.4);

  if (!post?.imageUrl) {
    return <ImagePlaceholder categoryId={post.category} aspect={previewAspect} rounded={rounded}/>;
  }

  return (
    <div style={{
      position: 'relative',
      paddingTop: `${(1 / previewAspect) * 100}%`,
      borderRadius: rounded ? '10px 10px 0 0' : 10,
      overflow: 'hidden',
      background: '#F2EDE6',
...
function PostCard({ post, onClick, onUserClick }) {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      {post.hasImage && <PostImage post={post}/>}
      ...
      <div style={{ padding: '10px 12px 12px' }}>
        ...
        <div style={{ display: 'flex', gap: 5, marginBottom: 7, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          {!post.isElite && <TagChip categoryId={post.category} small/>}
        </div>
